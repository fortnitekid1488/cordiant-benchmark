...
Не удаляйте и не переименовывайте его: это одно-листовый шаблон `Свод`, по которому собирается финальный результат. Если вы работаете с другой версией шаблона, замените этот файл новым workbook с тем же именем и одним листом `Свод`.
...
Это часть проекта и дефолтный одно-листовый шаблон `Свод` для сборки результата. Если workbook называется иначе, переименуйте его в это имя для работы через дашборд. CLI-скрипты также поддерживают параметр `--workbook`, но самый простой пользовательский путь ожидает стандартное имя.
...
- Исходный одно-листовый workbook `Бенч финансовой отчетности_мэйджоры.xlsx`; в дефолтной поставке он лежит прямо в репозитории.
