...
Не удаляйте его: это чистый одно-листовый шаблон `Свод`, по которому собирается финальный результат. В шаблоне должны оставаться структура, стили и заголовки, но не заполненные финансовые значения. Если вы работаете с другой версией шаблона, замените этот файл новым workbook с тем же видимым именем и одним листом `Свод`. Панель сама принимает оба технических Unicode-варианта буквы `й`, поэтому вручную угадывать невидимую форму имени не нужно.
...
```text
Installed. Use start_dashboard.cmd to open the dashboard, or update_from_github.cmd to update this folder later.
```
...
Это часть проекта и дефолтный чистый одно-листовый шаблон `Свод` для сборки результата. Если workbook называется иначе, переименуйте его в это видимое имя для работы через дашборд. CLI-скрипты также поддерживают параметр `--workbook`, а дашборд умеет находить стандартное имя в NFC/NFD-формах.
...
- Основной табличный источник для стандартных финансовых строк — StockAnalysis. Эти файлы загружаются в LLM первыми; официальные файлы добавляются только там, где StockAnalysis недоступен, не дает нужный период или не содержит нужный показатель.
- Браузер: Chrome, Edge, Safari, Brave или другой современный браузер.
- Аккаунт в выбранном LLM-сервисе нужен для основного извлечения JSON из подготовленных файлов. По умолчанию используется Qwen Studio, но в панели можно выбрать Google AI Studio, Kimi, Xiaomi MiMo или DeepSeek.
...
5. Дождитесь, пока появятся батчи. Каждый батч уже содержит только нужный набор источников: StockAnalysis first, официальные fallback-файлы только при необходимости.
...
14. Повторите шаги для нужных батчей.
15. Когда счетчик JSON станет `N/N`, нажмите `Собрать Excel`.
...
Важно: загружайте в LLM только файлы из `FILES_FOR_AI_STUDIO`. Там уже лежат StockAnalysis primary-файлы и только нужные fallback-документы. Не загружайте `source_manifest.json`, `aistudio_latest_quarter_schema.json`, `FILES_TO_UPLOAD.txt` и `prompt_for_aistudio.txt`; нужная структура уже встроена в промпт.
...
Excel-файл содержит один лист `Свод` в формате исходного бенча. Исходный workbook в репозитории тоже содержит только этот лист; отдельные листы компаний не нужны для пользовательского workflow. Обновленные значения получают комментарии с источниками, а рядом сохраняется machine-readable provenance JSON. Каждый новый запуск собирает итоговый файл из чистого шаблона и свежих источников; локальные заполненные образцы не используются как стартовая форма.
...
Корневая папка специально держит только пользовательские кнопки: install, start dashboard и update. Для диагностики используйте Python-команды ниже. Старые технические wrappers сохранены в `scripts/launchers/advanced/`, но обычному пользователю они не нужны.
...
├── update_from_github.cmd                    # обновление установленной папки на Windows
├── update_from_github.command                # обновление установленной папки на Mac
...
### В корне нет `OPEN_THIS_QUARTERLY_PACKAGE`

Это нормально. Корень проекта больше не засоряется package-ссылками. Дашборд и скрипты используют portable path-файлы в `outputs/latest_aistudio_*_package_path.txt`, поэтому workflow не зависит от symlink.
